...
Page::Page(lv_obj_t *panel)
{

  _first_station1 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station2 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station3 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station4 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station5 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station6 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station7 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station8 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station9 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station10 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
/*  _first_station11 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station12 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station13 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station14 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station15 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station16 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station17 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station18 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
  _first_station19 = new Station("Radio Paradise", "http://stream.radioparadise.com/aac-320");
*/
    
    createStyles();
// initial commands to create page: see RetroGUI::createPart
    panelPage = lv_obj_create(panel);
    lv_obj_set_size(panelPage, TFT_HOR_RES, 192);
    lv_obj_set_style_bg_color(panelPage, lv_color_hex(_colorMarker), 0);
    lv_obj_set_style_bg_opa(panelPage, LV_OPA_TRANSP, 0);
    lv_obj_set_style_pad_all(panelPage, 0, 0);
    lv_obj_set_style_border_color(panelPage, lv_color_hex(_colorStation), 0);
    lv_obj_set_style_border_width(panelPage, BORDER_WIDTH, 0);
    lv_obj_set_style_border_opa(panelPage, LV_OPA_COVER, 0);
    lv_obj_set_style_radius(panelPage, RADIUS, 0);
    createStationList(panelPage);
};

lv_obj_t* Page::get_lv_obj_panelPage()
{
  return panelPage;
};

void Page::createStyles()
{
    // NOW PLAYING
...
lv_obj_t *Page::createStationIndicator(lv_obj_t *parent)
{
    // ZEIGER
...
// end style
    ...
    lv_obj_remove_flag(ret, LV_OBJ_FLAG_SCROLLABLE);
...
lv_obj_t *Page::createStation(lv_obj_t *parent, const lv_font_t *font, bool inverse)
{
...
void Page::createStationList(lv_obj_t *parent)
{

    uint32_t stationY[28] = {3, 2, 4, 1, 5, 0, 6, 3, 2, 4, 1, 5, 0, 6, 3, 2, 4, 1, 5, 0, 6, 3, 2, 4, 1, 5, 0, 6}; // y pos of stations from left to right

/*    uint32_t numStations = _stations.size(); // getNumStations();

    if (numStations > MAX_STATIONS_PER_PAGE)
    {
        numStations = MAX_STATIONS_PER_PAGE;
    }
*/
uint32_t numStations = 0;
    ...
    int dy = numStations;
    int dx = MAX_STATIONS_PER_PAGE;

    int values[MAX_STATIONS_PER_PAGE] = {0};
...
//    for (uint8_t x = 0; x < MAX_STATIONS_PER_PAGE; x++)
...
/*            // REAL STATION
            typeStructTinyStation stat = _stations.at(stationCounter);
...
void RetroGUI::update(String json)
{
    ...
    // ROWS (static: one copy of the member is shared by all objects of the class)
    ...
    lv_obj_set_grid_cell(tl, LV_GRID_ALIGN_STRETCH, 0, 1, LV_GRID_ALIGN_STRETCH, 0, 1);
    ...
    lv_obj_set_size(panel, TFT_HOR_RES, 192);
    ...
    panelPage = new Page(panel);
//    panelPage2 = new Page(panel);
/*    panelPage3 = new Page(panel);
    panelPage4 = new Page(panel);*/

    // TOP - RIGHT
    tr = createPart(cont, false);
    lv_obj_set_grid_cell(tr, LV_GRID_ALIGN_STRETCH, 1, 1, LV_GRID_ALIGN_STRETCH, 0, 1);
...
/*    mid = createPart(panel, true);
    ...
    lv_obj_set_grid_cell(bot_upper, LV_GRID_ALIGN_STRETCH, 0, 2, LV_GRID_ALIGN_STRETCH, 2, 1);
    ...
    lv_obj_set_grid_cell(bot_lower, LV_GRID_ALIGN_STRETCH, 0, 2, LV_GRID_ALIGN_STRETCH, 3, 1);
    // BOTTOM_LINE
    ...
    // ZEIGER
    //station_indicator = createStationIndicator(panel);
    station_indicator = createStationIndicator(panelPage->get_lv_obj_panelPage());
...
void RetroGUI::begin(lv_obj_t *parent, String json)
{
...
void RetroGUI::setPage(uint8_t *page)
{
    _currentPage = *page;
}

void RetroGUI::setStations(typeArrStations *stations)
{
    _stations = *stations;
}

//  STATION
void RetroGUI::setStationPlaying(const char *station, bool store /* = true */)
{
    if (store)
    {
...
// TITLE
void RetroGUI::setTitlePlaying(const char *title, bool store /* = true */)
{
    if (store)
    {
        last_title = title;
...
void RetroGUI::setVolumeIndicatorMax(uint8_t maxValue)
{
    _maxVolume = maxValue;
}

// ANIMATION CALLBACK
void RetroGUI::anim_x_cb(void *var, int32_t v) // "static" removed
{
    lv_obj_set_x((lv_obj_t *)var, v);
}

void RetroGUI::setVolumeIndicator(uint8_t value)
{
...
bool RetroGUI::inRange(int val, int minimum, int maximum)
{
    return ((minimum <= val) && (val <= maximum));
}

int8_t RetroGUI::getStationIndexUnderIndicator(uint32_t indicatorPos)
{
...
void RetroGUI::setTuneIndicator(uint16_t &newPosition)
{
    if((newPosition >= 480) && (_currentPage < 2))
    {
      //gui_page_next();
      newPosition = 1;
      panelScroll(_currentPage++);
    }
    else if((newPosition == 0) && (_currentPage > 1))
    {
      //gui_page_prev();
      newPosition = 479;
      panelScroll(_currentPage--);
    ...
    //lv_obj_move_foreground(station_indicator);
}

void RetroGUI::tuneToStation(uint8_t station_id)
{
    uint32_t pw2 = POINTER_WIDTH / 2;
...
void RetroGUI::setVUMeterValue(uint16_t value)
{
...
void RetroGUI::updateDRDindicator(bool drdStopped)
{
    if (drdStopped)
    {
        if (tl)
...
uint32_t RetroGUI::getStationMidX(uint8_t stationIndex)
{
    return _stations[stationIndex].midX;
}

void RetroGUI::panelScroll(int Page)
{
    Serial.print("panelScroll: Page ");
    Serial.println(Page);
    if (Page == 1)
    {
        lv_obj_scroll_to_x(panel, 1, LV_ANIM_ON);
        lv_obj_set_parent(station_indicator, panelPage->get_lv_obj_panelPage());
    };
    if (Page == 2)
    {
        lv_obj_scroll_to_x(panel, 481, LV_ANIM_ON);
        lv_obj_set_parent(station_indicator, panelPage2->get_lv_obj_panelPage());
    };
    if (Page == 3)
    {
...
void RetroGUI::swapPage(uint8_t Page, uint8_t station, typeArrStations *arrStations)
{
    /*lv_obj_clean(mid);
    setPage(&Page);
...
/* Moved to class Page    // NOW PLAYING
    lv_style_init(&playing_style);
    lv_style_set_radius(&playing_style, 8);
    lv_style_set_border_color(&playing_style, lv_color_black());
    lv_style_set_border_width(&playing_style, 0);
...
lv_obj_t *RetroGUI::createPart(lv_obj_t *parent, bool border)
{
...
lv_obj_t *RetroGUI::createButtons(lv_obj_t *parent)
{
    lv_obj_update_layout(parent);
...
lv_obj_t *RetroGUI::createPlayingLabel(lv_obj_t *parent, const lv_font_t *font, bool top, bool inverse)
{
...
lv_obj_t *RetroGUI::createStationIndicator(lv_obj_t *parent)
{
...
lv_obj_t *RetroGUI::createVolumeIndicator(lv_obj_t *parent)
{
...
lv_obj_t *RetroGUI::createStation(lv_obj_t *parent, const lv_font_t *font, bool inverse)
{
...
void RetroGUI::createStationList(lv_obj_t *parent)
{
...
lv_obj_t *RetroGUI::createVolumeScale(lv_obj_t *parent, const lv_font_t *font, int min, int max)
{
...
void RetroGUI::create_vuMeter(lv_obj_t *parent)
{
...
#ifdef USE_PAGES                                          // SparkOfCode
void RetroGUI::_event_handler_buttonmatrix(lv_event_t *e) // "static" removed
{
...
#else // DrNeurosurg
static void RetroGUI::_event_handler_buttonmatrix(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    lv_obj_t *obj = (lv_obj_t *)lv_event_get_target(e);
    if (code == LV_EVENT_VALUE_CHANGED)
